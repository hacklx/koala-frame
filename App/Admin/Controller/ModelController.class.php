...
class ModelController extends AdminCoreController {
    ...
    public function build() {
        if (IS_POST) {
            ...
            $build = I('post.');
            //生成控制器
            if ($build ['build_action'] == 1) {
                $this->build_action();
            }
            //生成模型
            if ($build ['build_model'] == 1) {
                $this->build_model();
            }
            //生成菜单
            if ($build ['build_rule'] == 1) {
                $this->build_rule();
            }
            //生成index页面
            if ($build ['build_tpl_index'] == 1) {
                $this->build_tpl_index();
            }
            //生成add页面
            if ($build ['build_tpl_add'] == 1) {
                $this->build_tpl_add();
            }
            //生成edit页面
            if ($build ['build_tpl_edit'] == 1) {
                $this->build_tpl_edit();
            }
            //过程中无错误返回 成功信息
