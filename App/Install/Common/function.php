...
function register_administrator($db, $prefix, $admin, $auth){
    show_msg('开始注册创始人帐号...');
    $sql = "INSERT INTO `[PREFIX]user` (`id`, `username`, `nickname`, `password`, `last_login_time`, `last_login_ip`, `email`, `remark`, `amount`, `point`, `vip`, `overduedate`, `create_time`, `update_time`, `status`, `info`) VALUES (NULL, '[NAME]', '系统创始人', '[PASS]', '[TIME]', '[IP]', '[EMAIL]', NULL, '0.00', '0', '0', '0', '0', '0', '1', NULL)";
	
    $sql = str_replace(
        array('[PREFIX]', '[NAME]', '[PASS]', '[EMAIL]', '[TIME]', '[IP]'),
        array($prefix, $admin['username'], $admin['password'], $admin['email'], NOW_TIME, get_client_ip(1)),
        $sql);
    //执行sql
    $db->execute($sql);
    show_msg('创始人帐号注册完成！');
...
function update_tables($db, $prefix = ''){
    ...
    //替换表前缀
    $sql = str_replace(" `gms_", " `{$prefix}", $sql);
